...
public class CandidatureService {
    ...
    public void deposerCandidature(DeposerCandidatureRequest req) throws Exception {
        ...
        Saisonnier s = saisonnierRepo.findByCin(Integer.valueOf(req.cin)).orElse(null);
        ...
        if (candidatureRepo.existsBySaisonnierIdAndCampagneId(s.getId(), req.campagneId)) {
            throw new IllegalStateException("Vous avez déjà postulé à cette campagne");
        }
    ...
    private void creerUtilisateurSiAbsent(DeposerCandidatureRequest req, Saisonnier s) {
        // ✅ Vérifier par email ET par saisonnier pour éviter le doublon
        boolean existeParEmail      = utilisateurRepo.findByEmail(req.email).isPresent();
        boolean existeParSaisonnier = utilisateurRepo.findBySaisonnierIdOrEmail(s.getId(), req.email).isPresent();

        if (existeParEmail || existeParSaisonnier) return;
    ...
    public Map<String, Object> getParentByMatricule(String matricule, String email, String ip) {
        ...
        return Map.of(
                ...
                "email",     parent.getEmail() != null ? parent.getEmail() : "",  // ✅ ajout
    ...
    public void uploadParentsExcel(MultipartFile file, Long campagneId, String email, String ip) throws Exception {
        ...
        for (Row row : sheet) {
            ...
            String emailParent   = getCellValueAsString(row.getCell(3)).trim();  // ✅ nouveau
            ...
            if (!parentRepo.existsByMatriculeAndCampagneId(matricule, campagneId)) {
                ...
                parent.setEmail(emailParent);         // ✅ nouveau
